...
mixin InfluencerListMixin<T extends StatefulWidget> on State<T> {
  ...
  Widget buildInfluencerList(
      BuildContext context,
      List<SearchInfluencer> filteredInfluencers,
      bool isLoggedIn,
      String? email) {
                                ...
                                if (socialPrices != null)
                                  Padding(
                                    ...
                                    child: Wrap(
                                      ...
                                      runSpacing: 5.0,
                                      children:
                                          socialPrices.entries.map((entry) {
                                        final checkboxKey =
                                            '${influencer.id}_${entry.key}';
                                        final isInCart = cartSocials
                                                ?.contains(entry.key) ??
                                            false; // Check if the social site is in cartSocials
                                        print(
                                            "socialPrices matching influencer found for id: ${influencer.adminEmail} and email: $email");
                                        return Row(
                                          mainAxisSize: MainAxisSize.min,
                                          children: [
                                            if (isLoggedIn) // Only show checkbox if logged in
                                              (isInCart &&
                                                      influencer.adminEmail ==
                                                          email)
                                            ...
                                            Chip(
                                              avatar: CircleAvatar(
                                                ...
                                                child: Icon(
                                                  ...
                                                  color: influencerViewModel
                                                      .getSocialIconColor(
                                                          entry.key),
                                                  size:
                                                      18.0, // Set the icon size here
                                                ),
                                              ),
                                              label: Text(
                                                isLoggedIn
                                                    ? '${entry.value} ${influencer.socialCurrency}'
                                                    : '',
                                              ),
                                            ),
                                          ],
